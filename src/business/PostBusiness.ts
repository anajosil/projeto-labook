...
export class PostBusiness {
    ...
    public getPosts = async(
        input: GetPostsInputDTO): Promise<GetPostsOutputDTO> => {
        const { token} = input
        ...
        const postsDBwithCreator = await this.postDatabase.getPostsAndCreator()

      const posts = postsDBwithCreator.map((postAndCreator) => {
        const post = new Post(
            postAndCreator.id,
            postAndCreator.content,
            postAndCreator.likes,
            postAndCreator.dislikes,
            postAndCreator.created_at,
            postAndCreator.updated_at,
            postAndCreator.creator_id,
            postAndCreator.creator_name
    ...
    public editPost = async(
        ...
        if(!postDB){
            throw new NotFoundError ("O post com a id digitada não existe")
        }


        if(payload.id !== postDB.creator_id) {
            throw new ForbiddenError("O post poderá ser editado apenas pelo criador")
        }
    ...
    public deletePost = async(
        ...
        if (payload.role !== USER_ROLES.ADMIN){
          if(payload.id !== postDB.creator_id) {
            throw new ForbiddenError("Você não tem permissão para deletar o post")
        }
      }
    ...
    public likeOrDislikePost = async(
        ...
        if(!postDBWithCreatorName){
            throw new NotFoundError ("Post com o id informado não existe")
        }
